	...
	with mss.mss() as sct:
		...
		while 'Screen capturing':
			...
			if c & 0xFF == ord('q'):
				...
				grayth = cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_MEAN_C,\
				...
				h1, w1 = quest.shape[:2]
				h2, w2 = option1.shape[:2]
				h3, w3 = option2.shape[:2]
				h4, w4 = option3.shape[:2]

				vis = numpy.zeros(gray.shape[:2], numpy.uint8)
				vis[:h1, :w1] = quest
				vis[h1+10:h1+10+h2,:w2] = option2

				vis[h1+10+h2:h1+10+h2+h3,:w3] = option3

				vis[h1+10+h2+h3:h1+10+h2+h3+h4,:w4] = option4

				cv2.imwrite('screenshot.png',vis)
				cv2.imshow("test2",vis)
				cv2.waitKey(0)
